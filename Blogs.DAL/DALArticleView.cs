...
namespace Blogs.DAL
{
    public class DALArticleView : IDALArticleView
    {
        ...
        public IEnumerable<blog_view_article> GetList(ref GridPager pager)
        {
            ...
            List<blog_view_article> list = DataTableHelper.DataTableToModel<blog_view_article>(dt);

            return list;
        }

        public System.Data.IDataParameter CreateParameter(string parameterName, object parameterValue, System.Data.ParameterDirection? direction = null)
        {
            return DbInstance.CreateParameter(parameterName, parameterValue, direction);
        }


        public blog_view_article GetSingle(int articleID, string select = "*")
        {
            DataTable dt = DbInstance.GetDataTable("select " + select + " from blog_view_article where articleID=@articleID"
                , DbInstance.CreateParameter("@articleID", articleID));

            blog_view_article model = DataTableHelper.DataTableToModel<blog_view_article>(dt).FirstOrDefault();
